...
fn parse_nestjs_file(content: &str, file_path: &Path) -> Vec<DiscoveredEndpoint> {
    ...
    // @Controller('users')
    let controller_re =
        CONTROLLER_RE.get_or_init(|| Regex::new(r#"@Controller\s*\(\s*['"]([^'"]*)['"]"#).unwrap());

    // @Get(':id'), @Post('create'), @Post()
    let method_re = METHOD_RE.get_or_init(|| {
        Regex::new(r#"@(Get|Post|Put|Delete|Patch|All|Options|Head)\s*\(\s*(?:['"]([^'"]*)['"])?"#)
            .unwrap()
    });
...
"#;
        ...
        let get = endpoints
            .iter()
            .find(|e| e.method == "GET" && e.path == "/cats")
            .unwrap();
        assert!(
            get != endpoints
                .iter()
                .find(|e| e.method == "GET" && e.path == "/cats/:id")
                .unwrap()
        );
    }
}
